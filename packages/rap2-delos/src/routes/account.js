...
/**
router.post('/account/sso', async(ctx, next) => {
  console.log(ctx.request.body)
  let { empId, fullname = '', email = '' } = ctx.request.body
  ...
  if (!user) {
    user = await User.create({ empId, fullname, email })
  } else {
    await User.update({ fullname, email }, {
      where: { empId }
    })
  }
  Object.assign(ctx.session, { id: user.id, empId: user.empId, fullname: user.fullname, email: email })
  ctx.body = {
    data: user
  }
})
